...
class GraphSimulation(Simulation):
    ...
    def get_unemployed(self):
        return [p for p in self.population if p.is_unemployed()
                and p.status != Status.Death and p.infected_status == Status.Asymptomatic]
    ...
    def create_agent(self, status, social_stratum=None, recovering_time=0,infected_time=0):
        """
    ...
    def initialize(self):
        ...
        # Initial infecte mild population
        for i in np.arange(0, int(self.population_size * self.initial_infected_perc)):
            self.create_agent(Status. Infected_Mild, infected_time=5)

        # Initial infected critical population
        for i in np.arange(0, int(self.population_size * self.initial_infected_perc)):
            self.create_agent(Status.Infected_Critical, infected_time=5)
        
        # Initial infected severe population
        for i in np.arange(0, int(self.population_size * self.initial_infected_perc)):
            self.create_agent(Status.Infected_Severe, infected_time=5)

        # Initial immune population
        for i in np.arange(0, int(self.population_size * self.initial_immune_perc)):
            self.create_agent(Status.Recovered_Mild, recovering_time=7)
        
         # Initial immune population
        for i in np.arange(0, int(self.population_size * self.initial_immune_perc)):
            self.create_agent(Status.Recovered_Severe, recovering_time=14)
        
         # Initial immune population
        for i in np.arange(0, int(self.population_size * self.initial_immune_perc)):
            self.create_agent(Status.Recovered_Critical, recovering_time=30)
    ...
    def execute(self):
        ...
        for agent in filter(lambda x: x.status != Status.Death, self.population):
            ...
            if agent.status == Status.Asymptomatic:
    ...
    def contact(self, agent1, agent2):
        ...
        if agent1.status == Status.Susceptible and agent2.status == Status.Infected_Mild:
            ...
            if agent2.infected_time >= self.incubation_time + low \
                ...
                agent1.infection_status = Status.Exposed
                if contagion_test <= self.contagion_rate:
                    agent1.status = Status.Infected_Mild
                    agent1.infection_status = Status.PreSymptomatic
