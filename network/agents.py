...
from covid_abs.agents import Agent, AgentType, Status
...
class Business(Agent):
    ...
    def hire(self, agent):
        if agent.status != Status.Death and agent.status == Status.Asymptomatic:
    ...
    def accounting(self):
        ...
        if self.type == AgentType.Business:
            ...
            if 2 * (labor + tax) < self.incomes:
                ...
                # Verificar se a lista 'unemployed' está vazia
                if len(unemployed) == 0:
                    return  # Retorna da função se não houver desempregados
                ix = np.random.randint(0, len(unemployed))
                self.hire(unemployed[ix])
            elif (labor + tax) > self.incomes:
                if self.num_employees > 0:  # Certifique-se de que há funcionários para demitir
...
class Person(Agent):
    ...
    def __init__(self, **kwargs):
        ...
        self.type = AgentType.Individual
    ...
    def move_to_work(self):
        ...
        if self.infected_status == Status.Death:
            return
        
        if self.infected_status == Status.Infected_Critical:
            return
        
        if self.infected_status == Status.Infected_Severe:
            return
    ...
    def move_to_home(self):
        ...
        if self.infected_status == Status.Death:
            print(f'entrou no infectado status = { self.infected_status}')
            return
        ...
        if self.house is not None:
            print(f'dentro do if house = {self.house}')
    ...
    def move_freely(self):
        ...
        x,y = np.random.normal(0, self.environment.amplitudes['I1'], 2)
        self.x = int(self.x + x)
        self.y = int(self.y + y)
    ...
    def update(self):
        ...
        """
        print(f'status update {self.status}')
        ...
        if self.status == Status.Infected_Mild or self.status == Status.Infected_Critical or self.status == Status.Infected_Severe:
            ...
            if self.infected_status == Status.Infected_Mild:
                if age_hospitalization_probs[ix] > test_sub:
                    self.infected_status = Status.Infected_Severe
                    self.move_to(self.environment.healthcare)
            elif self.infected_status == Status.Infected_Severe:
                if age_severe_probs[ix] > test_sub:
                    self.infected_status = Status.Infected_Critical
                    stats = self.environment.get_statistics(kind='info')
                    print(f'status dentro do infectado -- >{stats}')
                    if stats['Infected_Critical'] >= self.environment.critical_limit:
                        self.status = Status.Death
                        self.infected_status = Status.Infected_Mild
                        if self.house is not None:
            ...
            if age_death_probs[ix] > death_test:
                self.status = Status.Death
                self.infected_status = Status.Asymptomatic
                self.move_to_home()
                return

            if self.infected_time > self.environment.recovering_time:
                self.infected_time = 0
                self.status = Status.Recovered_Critical
                self.infected_status = Status.PreSymptomatic
